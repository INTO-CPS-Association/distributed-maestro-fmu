...
sessionResult=$(curl -s -H "Content-Type: application/json" http://localhost:8082/createSession)

echo Got response $sessionResult

sessionId=`echo $sessionResult | cut -d ":" -f 2- | cut -d "}" -f 1`

echo Session $sessionId

# call initialize
echo initializing
initResult=$(curl -s -H "Content-Type: application/json" --data @$1 http://localhost:8082/initialize/$sessionId)
